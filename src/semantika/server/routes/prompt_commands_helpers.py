...
async def execute_template_flow(data: dict[str, Any]) -> dict[str, Any]:
    ...
    turn1_system = (
        ...
        "## Nodes\n"
        "Templates often reference fixed nodes (not parameters) in their "
        "triple patterns. For example, ``{subject} rdf:type Scientist`` "
        "references a ``Scientist`` node; ``{subject} rs:worksAt Lab`` "
        "references a ``Lab`` node. "
