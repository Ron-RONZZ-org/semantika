...
// Mock the cowrite module so the .svelte component import doesn't fail
vi.mock("@lightercore/ui/cowrite/index.js", () => ({
  ...
  CowriteButton: function DynamicFormCowriteBtnMock() {},
  CowritePanel: function DynamicFormCowritePanelMock() {},
}));
// Also mock the individual .svelte file imports to avoid compilation errors
vi.mock("@lightercore/ui/cowrite/CowriteButton.svelte", () => ({
  default: function CowriteButtonMock() {},
}));
vi.mock("@lightercore/ui/cowrite/CowritePanel.svelte", () => ({
  default: function CowritePanelMock() {},
}));
...
describe("DynamicForm", () => {
  ...
  // ── Cowrite integration: verify the cowrite module loads via DynamicForm ─

  it("initializes without errors when cowrite module is mocked", () => {
    // This test verifies that importing @lightercore/ui/cowrite/index.js
    // in DynamicForm's script section doesn't cause runtime errors.
    // If the mock is set up correctly, the Save button should still render.
    render(DynamicForm, {
      props: { commandPath: ["node", "add"] },
    });
    expect(screen.getByText("Save")).toBeTruthy();
  });
});
