...
- **Specialised `!node add` subcommands**: `photo`, `video`, `file`, `code` replace the old `--img/--attachment/--file/--in-place/--move` flags. Each auto-creates a node with the appropriate `rdf:type` (sm:Photo, sm:Video, sm:Document, sm:SourceCode), file metadata triples, and domain-specific triples (`sm:depicts`, `sm:theme`, `sm:programmingLanguage`, `sm:dimension`, `sm:canonicalLink`). The general `!node add` is kept for label-only creation + arc shortcuts.
  - **Inline code support** (`node.add.code`): The ``--code`` flag accepts inline source code text, stored directly in the DB ``code_content`` column (FTS5-searchable). This replaces the file-only requirement — ``--path`` is now optional (``group: "source"`` toggle in the GUI). ``--no-copy`` is removed for the code subcommand (text-only code has no file to copy).
  - **Language autocomplete**: The ``--lang`` flag supports a ``suggestions`` field with common programming languages (python, javascript, typescript, etc.) rendered as a ``<datalist>`` in the GUI form.
...
- **Interactive commands**: Commands with `interactive: true` in `tree.py` trigger a form popup in the GUI when required params are missing.
  - **Field placeholders**: All ``add`` commands now include ``placeholder`` text in param/flag metadata showing example input (e.g. ``"en::Albert Einstein, eo::Albert Ejnŝtejno"`` for labels).
