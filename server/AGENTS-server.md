...
- **Cowrite route** (`POST /api/v1/cowrite`): Accepts ``form_type``, ``fields``, ``instruction`` from the frontend. Loads the cowrite style cascade via ``lightercore.cowrite.style.load_cowrite_style`` with semantika-specific defaults from ``core/cowrite_defaults.py``. Uses the singleton ``LLMProvider.chat()`` directly (bypassing the tool loop and system prompt injection — cowrite builds its own system message). Delegates engine execution to ``lightercore.cowrite.engine.cowrite``. Validation errors return 400; LLM unavailability returns 502; LLM response parse errors return 422.
