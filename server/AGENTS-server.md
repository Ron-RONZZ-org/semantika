...
- **Cowrite route** (`POST /api/v1/cowrite`): Accepts ``form_type``, ``fields``, ``instruction`` from the frontend. Loads user style from ``AGENTS.md`` via :func:`semantika.server.llm.system_prompt.load_user_style` (shared with the main LLM agent — one file for all style customisation). Builds a JSON schema from the form fields and wraps ``provider.chat()`` with structured output enforcement (``response_format``), falling back gracefully when the provider doesn't support strict schemas. Delegates execution to ``lightercore.cowrite.engine.cowrite``. Validation errors return 400; LLM unavailability returns 502; LLM response parse errors return 422.
