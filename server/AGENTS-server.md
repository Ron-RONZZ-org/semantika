...
- **Template turn prompts use named-only expansion**: ``_expand_turn_prompt`` in ``prompt_commands_helpers.py`` uses pure ``str.replace()`` with a dict of named variables (e.g. ``$AVAILABLE_PREDICATES``, ``$TEMPLATE_DESCRIPTION``, ``$STYLE_EXAMPLE``, ``$ARGUMENTS``). Turn prompts do **not** use positional ``$1``/``$2`` expansion — unknown placeholders are left as-is so old-style files visibly fail rather than silently producing wrong output.
- **Template flow multi-phase resume**: The ``/template`` command is a two-turn LLM flow (predicate discovery → YAML generation). Both turns can gate WRITE-level tools (``predicate.add`` in turn 1, ``template.save`` in turn 2) behind HITL confirmation via ``_pending_executions`` session state. The ``template_flow`` annotation on the session (set by ``_annotate_template_flow``) signals to ``resume_execution`` that after turn 1's tool loop completes, turn 2 should be started automatically via ``_run_template_turn2`` rather than returning a chat response.
- **Style example injection in turn 2**: ``_get_style_example()`` scans ``~/.config/semantika/templates/*.{yaml,yml}`` and returns the most recently modified template as a YAML style example. This is injected as ``$STYLE_EXAMPLE`` in the turn 2 prompt. If no templates exist, the placeholder becomes an empty string and the section is omitted.
- **Tool sets for template turns**: Turn 1 includes ``predicate.search`` (READ) and ``predicate.add`` (WRITE) so the LLM can both find and create predicates. Turn 2 includes ``template.save`` (WRITE), ``template.list`` (READ), ``template.view`` (READ), and ``predicate.search`` (READ).
